...
import org.example.interfaces.ILeiloavel;
import org.example.interfaces.IMercadoLeilao;
import org.example.interfaces.IUsuario;
import org.example.interfaces.IVendido;
...
public class MercadoLeilao implements IMercadoLeilao, Serializable {
	...
	public List<? extends ILeiloavel> getProdutosEmLeilao() {
		...
		retornoProdutosEmLeilao.addAll((Collection<? extends ILeiloavel>) this.produtosEmLeilao);
	...
	public List<? extends ILeiloavel> getProdutosVencidosENaoVendidos() {
		...
		retornoProdutosVencidos.addAll((Collection<? extends ILeiloavel>) this.produtosVencidosENaoVendidos);
	...
	public List<? extends IVendido> getProdutosVendidos() {
		...
		retornoProdutosVendidos.addAll((Collection<? extends IVendido>) this.produtosVendidos);
	...
	public List<IUsuario> getUsuariosCadastrados() {
		...
		retornoUsuariosCadastrados.addAll((Collection<? extends IUsuario>) this.usuarios.values());
	...
	public IUsuario getUsuarioPor(String cpf) throws Exception {
		IUsuario usuario = (IUsuario) this.usuarios.get(cpf);
		if (usuario == null)
			throw new Exception("O usuario nao esta cadastrado.");
		return usuario;
	}
	...
	private List<? extends ILeiloavel> getProdutosEmLeilaoQueDeuLance(String cpf) {
		...
		for (int i = 0; i < produtosEmLeilao.size(); i++) {
			...
			if (verificaSeDeuLanceNesseProduto(cpf, produto))
				retornoProdutos.add((ILeiloavel) produto);
		}
		return retornoProdutos;
	}
